...
def home():
	...
	return render_template('index.html',posts=post,title='ketan\' blog')
...
def register():
	...
	if form.validate_on_submit():
		...
		db.session.add(user)
		db.session.commit()
		flash('Your Account Has been  Created For !! You Can Now Log In','success')
...
def save_picture(form_picture):
	random_hex= binascii.hexlify(os.urandom(16)).decode('utf-8')
	_, f_ext = os.path.splitext(form_picture.filename)
	picture_fn = random_hex + f_ext
	picture_path = os.path.join(app.root_path,'static/profile_pics',picture_fn)
	
	output_size = (125,125)
	small_image = Image.open(form_picture)
	small_image.thumbnail(output_size)

	small_image.save(picture_path)
